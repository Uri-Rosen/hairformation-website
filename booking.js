document.addEventListener('DOMContentLoaded', function() {
    ...
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.attributeName === 'class' && step2.classList.contains('active')) {
                setTimeout(() => $('#date').datepicker('show'), 50);
            }
        });
    });
        ...
        if (prevBtn) {
            prevBtn.addEventListener('click', function() {
                ...
                if (prevStep) {
                    ...
                    if (i - 1 === 2) setTimeout(() => $('#date').datepicker('show'), 50);
                }
            });
        }
    ...
    bookingForm.addEventListener('submit', async function(event) {
        ...
        try {
            ...
            if (data.error) {
                ...
                const appointmentDetails = { firstName, lastName, date, time, haircutType: serviceType };
    ...
    function validateForm() {
        ...
        if (!haircutTypeSelect.value) {
            ...
            valid = false;
        } else removeValidationError(haircutTypeSelect);

        if (!dateInput.value) {
            showValidationError(dateInput, 'אנא בחרו תאריך.');
            valid = false;
        } else removeValidationError(dateInput);

        if (!timeSelect.value) {
            showValidationError(timeSelect, 'אנא בחרו שעה.');
            valid = false;
        } else removeValidationError(timeSelect);
        ...
        if (!phoneVal) {
            ...
            valid = false;
        } else removeValidationError(phoneInput);
